...
describe("sebuMaster - function tests", function() {
    ...
    it("init()", async function() {
        let sebu2 = await deploy("SebuMaster",fee,token.target,accounts[1].address,shepard.address);
        assert(await sebu2.portfolio() == ethers.ZeroAddress)
        await expect(sebu2.init(portfolio.target,fundingContract.target)).to.be.reverted;//only guardian
        await sebu2.connect(accounts[1]).init(portfolio.target,fundingContract.target)
    ...
    it("setRanking()", async function() {
        await token.connect(accounts[2]).approve(sebu.target,ethers.parseEther("5", 'wei'))
        await sebu.connect(accounts[2]).pitch(samplePitch.target);
        await expect(sebu.connect(accounts[2]).setRanking(100)).to.be.reverted;//must be shepard
